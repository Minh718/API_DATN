...
public class CartService {
        ...
        public Page<CartProductSizeColorDTO> getAllProductOfCart(int page, int size) {
                ...
                Page<CartProductSizeColor> cartItems = cartProductSizeColorRepository
                                .findAllByCartIdOrderByUpdateAtDesc(user.getCart().getId(), pageable);

                List<CartProductSizeColorDTO> dtoList = new ArrayList<>();

                for (CartProductSizeColor item : cartItems) {
                        Long productSizeColorId = item.getProductSizeColorId();
                        Integer availableQuantity = (Integer) redisService
                                        .getKey("productSizeColor:" + productSizeColorId);

                        if (availableQuantity == null || availableQuantity <= 0) {
                                cartProductSizeColorRepository.delete(item);
                                continue;
                        }

                        if (item.getQuantity() > availableQuantity) {
                                item.setQuantity(availableQuantity);
                                item.setUpdateAt(DateTimeUtil.getCurrentVietnamTime());
                                cartProductSizeColorRepository.save(item);
                        }
                        ...
                        CartProductSizeColorDTO dto = CartProductSizeColorMapper.INSTANCE
                                        .toCartProductSizeColorDTO(item);

                        ProductSizeColorDTO productSizeColorDTO = ProductSizeColorMapper.INSTANCE
                                        .toProductSizeColorDTO(productSizeColor);

                        productSizeColorDTO.setQuantity(availableQuantity);

                        Integer sizeQuantity = (Integer) redisService.getKey(
                                        "productSize:" + productSizeColor.getProductSize().getId());

                        productSizeColorDTO.getProductSize().setQuantity(sizeQuantity != null ? sizeQuantity : 0);
                        dto.setProductSizeColor(productSizeColorDTO);

                        dtoList.add(dto);
                }

                return new PageImpl<>(dtoList, pageable, cartItems.getTotalElements());
        }
}